...
async function poller() {
  ...
  try {
    ...
    // Choix source données drone (simulation/API)
    ...
    // Construction de la liste complète enrichie des vols détectés
    for (const drone of drones) {
      ...
      // Ajout de la trace et état dans les données complètes du drone
      const fullDroneData = {
        ...drone,
        trace,
    ...
    // Récupération et enrichissement de tous les vols connus (live + waiting) avec leur trace
    ...
    log.info(`[poller] Broadcasting ${flightsToBroadcast.length} drones (live + waiting) after filtering local`);

    // Diffuser la liste des vols actualisés à tous les clients WebSocket
